...
│   └── chroma.sqlite3          
└── doc/
    ├── doc_scangestor/         # Documentación fuente para RAG
    │   ├── FUNCIONAL/          # Docs de funcionalidades
    │   ├── TECNICA/            # Docs técnicos
    │   └── GESTION/            # Docs de gestión
    └── doc_capstone/           # Documentación del proyecto (este directorio)
...
```mermaid
Usuario → Orquestador → Categoría detectada → Agente Especializado → ChromaDB → LangChain → Respuesta
```
...
```mermaid
Usuario → Orquestador → LEXICA detectada → 3 Agentes en paralelo → Búsqueda en archivos → Sintetizador → Respuesta unificada
```
